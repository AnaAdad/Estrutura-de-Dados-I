...
int main(){
    double valormercadoria, entrada, prestacao;
    
    printf("Digite o valor do produto: ");
    scanf("%lf", &valormercadoria);
    
    if(valormercadoria >= 3){
        entrada += 1;
        prestacao -= 1;
    }else{
        entrada = valormercadoria / 2;
        prestacao = valormercadoria - entrada;
    }
    
    printf("\nA entrada na loja eh de R$: %.2f\n", entrada);
    printf("As duas prestaoes sao de R$: %.2f\n", prestacao);
